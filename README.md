...
- An optional **geo location whitelist** (admin Settings panel — a list of allowed country codes) can further restrict every `/watch` and `/bundle` page, checked before the email gate. Empty by default (no restriction); detected via Vercel's edge network, so it fails open (never blocks) off Vercel or in local dev.
- The **admin page itself** can also be geo-restricted, via the `ADMIN_GEO_WHITELIST` env var plus a Settings-panel toggle to enforce it. The country list is env-var-only (not editable in the UI) precisely so a lockout is always recoverable from your hosting dashboard rather than trapped behind the page it protects.
...
| `ADMIN_GEO_WHITELIST` | Optional comma/space-separated ISO country codes (e.g. `US, CA`) allowed to reach the admin page/API, on top of the credentials above. Also requires enabling it in the Settings panel (off by default). Deliberately an env var rather than a Settings field, so a misconfiguration is always fixable from your hosting dashboard, not locked behind the very page it protects. |

...
- You can optionally **watermark** the recipient's verified email across the player to deter and attribute leaks. It's controlled in layers — a global default (admin **Settings** panel), a per-share Always/Never override, and exempt emails/domains that are never watermarked (e.g. internal admins) — resolved as exemption → per-share → global default. It's a client-side overlay for attribution, not burned into the video, so it's a deterrent, not DRM.

...
const DEFAULTS = {
  ...
  watermarkByVideo: {},
  // Global geo whitelist: ISO 3166-1 alpha-2 country codes (e.g. "US", "CA").
  // Empty = no restriction (default). See lib/geo.js for how it's enforced
  // and its honest limits (Vercel-only signal, fails open elsewhere).
  geoWhitelistCountries: [],
...
export async function saveSettings(patch = {}) {
  ...
  const next = {
    ...
    geoWhitelistCountries: normalizeCountryList(
      patch.geoWhitelistCountries === undefined
        ? current.geoWhitelistCountries
        : patch.geoWhitelistCountries
    ),
...
// Keeps only well-formed 2-letter codes, uppercased (matches the header
// format lib/geo.js compares against) — anything else typed into the
// Settings field is silently dropped rather than stored malformed.
function normalizeCountryList(v) {
  return [
    ...new Set(
      toList(v)
